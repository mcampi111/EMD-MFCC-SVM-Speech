kkfunction_MFCC_raw.k <- function(sigma_rbf1, sigma_rbf2, sigma_rbf3, sigma_rbf4, sigma_rbf5, w1,w2,w3,w4,w5){
  k <- function (x,y){  #y = NULL
    
    x1<- x[grep("x_syntspeak1_MLFC_coeff_7",names(x))]
    y1<- y[grep("x_syntspeak1_MLFC_coeff_7",names(y))]
    k1<- as.numeric(w2)*exp( sigma_rbf1 * (2 * crossprod(x1, y1) - crossprod(x1) - crossprod(y1)) )
    
    x2<- x[grep("x_syntspeak1_MLFC_coeff_8",names(x))]
    y2<- y[grep("x_syntspeak1_MLFC_coeff_8",names(y))] 
    k2<- as.numeric(w2)*exp( sigma_rbf2 * (2 * crossprod(x2, y2) - crossprod(x2) - crossprod(y2)) )
    
    x3<- x[grep("x_syntspeak1_MLFC_coeff_9",names(x))]
    y3<- y[grep("x_syntspeak1_MLFC_coeff_9",names(y))] 
    k3<- as.numeric(w3)*exp( sigma_rbf3 * (2 * crossprod(x3, y3) - crossprod(x3) - crossprod(y3)) )
    
    x4<- x[grep("x_syntspeak1_MLFC_coeff_10",names(x))]
    y4<- y[grep("x_syntspeak1_MLFC_coeff_10",names(y))] 
    k4<- as.numeric(w4)*exp( sigma_rbf4 * (2 * crossprod(x4, y4) - crossprod(x4) - crossprod(y4)) ) 
    
    x5<- x[grep("x_syntspeak1_MLFC_coeff_11",names(x))]
    y5<- y[grep("x_syntspeak1_MLFC_coeff_11",names(y))] 
    k5<- as.numeric(w2)*exp( sigma_rbf5 * (2 * crossprod(x5, y5) - crossprod(x5) - crossprod(y5)) ) 

...
for(i in (1:m)){
  ...
  feat_speak1_mean[[i]]<- rollapply(data_speak1[i,],width = 6000, by = 6000, FUN =mean, align = 'left')
...
for(i in (1:m)){
  ...
  feat_speak1_var[[i]]<- rollapply(data_speak1[i,],width = 6000, by =6000, FUN =var, align = 'left')
...
for(i in (1:m)){
  ...
  feat_speak1_skew[[i]]<- rollapply(data_speak1[i,],width = 6000, by =6000, FUN =skewness, align = 'left')
...
for(i in (1:m)){
  ...
  feat_speak1_kurt[[i]]<- rollapply(data_speak1[i,],width = 6000, by =6000, FUN =kurtosis, align = 'left')
...
for(i in (1:m)){
  ...
  feat_speak1_min[[i]]<- rollapply(data_speak1[i,],width = 6000, by =6000, FUN =min, align = 'left')
...
for(i in (1:m)){
  ...
  feat_speak1_max[[i]]<- rollapply(data_speak1[i,],width = 6000, by =6000, FUN =max, align = 'left')
...
for(i in (1:m)){
  ...
  feat_speak1_rms[[i]]<- rollapply(data_speak1[i,],width = 6000, by =6000, FUN =rms, align = 'left')
...
speak1_vec<- lapply(1:m, function(k){
  ...
  speak1_vec2[[k]]<- sapply(1:length(feat_speak1_mean[[k]]), function(i){
      ...
      cbind(feat_speak1_mean[[k]][i], feat_speak1_var[[k]][i], 
            ...
            feat_speak1_kurt[[k]][i], feat_speak1_min[[k]][i], feat_speak1_max[[k]][i],  
            feat_speak1_rms[[k]][i]) 
